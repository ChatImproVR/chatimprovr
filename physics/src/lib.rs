...
pub struct RapierContext {
    ...
    #[cfg_attr(feature = "serde-serialize", serde(skip))]
    pub(crate) last_body_transform_set: HashMap<RigidBodyHandle, cimvr_common::Transform>,
...
impl RapierContext {
    ...
    /// returns the `EntityId` containing that rigid-body.
    pub fn collider_parent(&self, entity: EntityId) -> Option<EntityId> {
    ...
    pub fn collider_entity(&self, handle: ColliderHandle) -> Option<EntityId> {
    ...
    pub(crate) fn collider_entity_with_set(
        ...
        colliders.get(handle).map(|c| EntityId(c.user_data))
    }

    /// Retrieve the Bevy entity the given Rapier rigid-body (identified by its handle) is attached.
    pub fn rigid_body_entity(&self, handle: RigidBodyHandle) -> Option<EntityId> {
        self.bodies.get(handle).map(|c| EntityId(c.user_data))
    }
    ...
    fn with_query_filter_elts<T>(
        entity2collider: &HashMap<EntityId, ColliderHandle>,
        entity2body: &HashMap<EntityId, RigidBodyHandle>,
        colliders: &ColliderSet,
        filter: QueryFilter,
        f: impl FnOnce(RapierQueryFilter) -> T,
    ) -> T {
        ...
        if let Some(predicate) = filter.predicate {
            let wrapped_predicate = |h: ColliderHandle, _: &rapier3d::geometry::Collider| {
    ...
    pub fn entity2body(&self) -> &HashMap<EntityId, RigidBodyHandle> {
        &self.entity2body
    }

    /// The map from entities to collider handles.
    pub fn entity2collider(&self) -> &HashMap<EntityId, ColliderHandle> {
        &self.entity2collider
    }

    /// The map from entities to impulse joint handles.
    pub fn entity2impulse_joint(&self) -> &HashMap<EntityId, ImpulseJointHandle> {
        &self.entity2impulse_joint
    }

    /// The map from entities to multibody joint handles.
    pub fn entity2multibody_joint(&self) -> &HashMap<EntityId, MultibodyJointHandle> {
    ...
    pub fn cast_ray_and_get_normal(
        ...
        solid: bool,
        filter: QueryFilter,
    ) -> Option<(EntityId, RayIntersection)> {
        let ray = Ray::new(
            (ray_origin / self.physics_scale).into(),
            (ray_dir / self.physics_scale).into(),
        );
    ...
    pub fn intersections_with_ray(
        ...
        solid: bool,
        filter: QueryFilter,
        mut callback: impl FnMut(EntityId, RayIntersection) -> bool,
    ) {
        let ray = Ray::new(
            (ray_origin / self.physics_scale).into(),
            (ray_dir / self.physics_scale).into(),
        );
        let callback = |h, inter: rapier3d::prelude::RayIntersection| {
            self.collider_entity(h)
    ...
    pub fn intersections_with_point(
        ...
        point: Vect,
        filter: QueryFilter,
        mut callback: impl FnMut(EntityId) -> bool,
    ) {
    ...
    pub fn project_point_and_get_feature(
        ...
        point: Vect,
        filter: QueryFilter,
    ) -> Option<(EntityId, PointProjection, FeatureId)> {
